...
class PocketBot {
    ...
    async start() {
        ...
        console.log(`⚠️  IMPORTANT: Please manually set Contract Expiration to M5 (00:05:00) on the UI now.`);
        console.log(`⚠️  Bot will analyze 1m candles for 5m expiration trades.`);
        
        const stats = this.learner.getStats();
        console.log(`🧠 Learning Stats: ${stats.totalTrades} trades, Win Rate: ${stats.globalWinRate}`);
    ...
    async loop() {
        while (true) {
            try {
                // 1. Check Limits
                if (this.totalPnL >= CONFIG.targetProfit) { console.log('✅ Target Profit. Stop.'); break; }
                if (this.totalPnL <= -CONFIG.stopLoss) { console.log('❌ Stop Loss. Stop.'); break; }
                ...
                if (action && (action.includes('BUY') || action.includes('SELL'))) {
                    ...
                    if (wisdom.allowed) {
                        console.log(`🚀 Signal: ${action} | Brain: ${wisdom.reason} -> EXECUTING`);
                        ...
                        console.log(`✋ Signal: ${action} | Brain: REJECTED (${wisdom.reason})`);
                    }
                } else {
    ...
    async getSignal() {
        try {
            ...
            const res = await fetch(`http://localhost:3000/api/signal?symbol=${CONFIG.symbol}&interval=1`);
    ...
    async executeTrade(direction: 'CALL' | 'PUT') {
        const amount = this.currentStep === 0 
            ? CONFIG.baseAmount 
            : CONFIG.baseAmount * Math.pow(CONFIG.martingaleMultiplier, this.currentStep);

        console.log(`⚡ ${direction} | $${amount.toFixed(2)} | Step ${this.currentStep}`);
    ...
    async checkResult() {
        // TODO: Use OCR or DOM scraping to get real result
        // For simulation/demo, we randomly win 55% of the time to test logic
        return Math.random() > 0.45 ? 'WIN' : 'LOSS';
    }
